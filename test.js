...
app.use((err, req, res, next) => {
  if (err.name === 'MulterError') {
    ...
    return The task is to update indexmaster.js by incorporating improvements from index.js and temp.js. Use indexmaster.js as the base structure and authority, but when duplicate functions exist, select the best implementation. Maintain indexmaster.js's architecture while adding any missing features from the other files.

First, I need to understand the files:

- indexmaster.js: This is the base file. It's a complete Express.js server setup for a Car Crash Lawyer AI application, with various endpoints, middleware, Supabase integration, etc.

- index.js: This seems to be an enhanced version with GDPR improvements, including new endpoints like /api/gdpr/log-consent, /api/gdpr/export/:userId, /api/debug/webhook-test.

- temp.js: This appears similar to index.js, with some parts truncated, but it includes the same new GDPR endpoints.

The user message has <DOCUMENT> tags for indexmodern.js, indexmaster.js, and temp.js. indexmodern.js seems to be the "index.js" referred to.

The query is: "Update indexmaster.js by incorporating improvements from index.js and temp.js."

But in the documents:

- "indexmodern.js" is labeled as "index.js" in the code to update.

- Then "indexmaster.js"

- Then "temp.js"

I think "index.js" refers to the content in "indexmodern.js", which is the enhanced version.

Temp.js has similar enhancements.

The goal is to take indexmaster.js as base, and add the missing features from the others, like the new GDPR endpoints.

Key missing features in indexmaster.js compared to the others:

From indexmodern.js and temp.js:

1. Enhanced GDPR Consent Logging Endpoint: /api/gdpr/log-consent

2. Debug Webhook Test Endpoint: /api/debug/webhook-test

3. GDPR Data Export Endpoint: /api/gdpr/export/:userId

Also, in indexmaster.js, there are some GDPR related things, but not these specific endpoints.

Indexmaster.js has:

- checkGDPRConsent middleware

- logGDPRActivity function

But no /api/gdpr/log-consent, no /api/debug/webhook-test, no /api/gdpr/export/:userId

It does have /api/gdpr/anonymize/:userId and others? Wait, looking at the truncated part.

In indexmaster.js, it's truncated, but from the content, it has webhook endpoints, pdf generation, etc.

To update:

- Use indexmaster.js as base.

- When duplicates, choose the best.

- Add missing features.

I need to add those three endpoints to indexmaster.js.

Place them after the existing GDPR sections, as suggested.

In indexmaster.js, look for where to add.

In indexmaster.js, there is:

// Delete user data

app.post('/api/gdpr/delete/:userId', ...)

// Anonymize user data

app.post('/api/gdpr/anonymize/:userId', ...)

Then it ends with // ========================================

But in the document, it's truncated.

The full content is in the <DOCUMENT filename="indexmaster.js"> ... </DOCUMENT>

It has many endpoints, including webhooks, pdf generation, etc.

To add the new ones, I should add them in the GDPR section.

Also, in indexmodern.js, there are enhancements like validation in log-consent, added consent_history and audit_logs in export, etc.

I need to select the best implementations.

For the response, I need to deliver the complete updated file in an artifact, plus summary, note, etc.

The requirements are:

1. Complete updated code in an artifact
...
4. Code Style: Clean, etc.

Since this is about GDPR, it's safe as it's for compliance.

Plan:

- Copy indexmaster.js as base.

- Add the three endpoints from indexmodern.js or temp.js. They are similar, but indexmodern.js has some extra validations and enhancements.

- In log-consent, indexmodern.js has additional validation for consent_given being boolean.

- In export, indexmodern.js has more data collected: consent_history, audit_logs, and query param for format.

- In debug webhook, more analysis.

Use the versions from indexmodern.js as they seem more enhanced.

- Place them after the anonymize endpoint or in a new section.

In indexmaster.js, after anonymize, it's truncated, but probably add there.

Looking at the content, indexmaster.js has:

After anonymize, then // PDF Generation Endpoint

No, in the provided text, it's:
...
To add, I can add a section // NEW ENHANCED GDPR ENDPOINTS like in indexmodern.js.

Add them there.

Ensure CONSTANTS has GDPR.CONSENT_TYPES if used.

In log-consent, it uses CONSTANTS.GDPR.CONSENT_TYPES.DATA_SHARING

In indexmaster.js, CONSTANTS doesn't have GDPR.

So, add that.

In indexmodern.js, it has CONSTANTS with GDPR: { CONSENT_TYPES: ... }

Add that to indexmaster.js.

Also, in indexmaster.js, Logger is defined, same.

In startup log, add the new endpoints.

To keep architecture, add them in the appropriate place.

For duplicates: for example, the debug/user endpoint is in both.

In indexmaster.js, it's there.

The webhook-test is new.

Also, in indexmaster.js, there is no /api/gdpr/log-consent, etc.

In indexmaster.js, the webhook/signup has consent handling.

But the new log-consent is separate.

I think it's fine to add.

For the final output, wrap in <artifact> tags.

Also, provide summary and note.<artifact identifier="enhanced-indexmaster-js" type="application/vnd.ant.code" language="javascript" title="Enhanced indexmaster.js with GDPR Improvements">
// ========================================
// Car Crash Lawyer AI - Main Server
// Version: 2.0.0
// Last Updated: 2024
...
- Added three new GDPR-related endpoints: enhanced consent logging, data export, and webhook debug test, incorporating validations and comprehensive data handling from the improved implementations.
- Extended CONSTANTS with GDPR consent types for better type safety and maintainability.
- Enhanced the export endpoint to include consent history and audit logs, with support for future formats.
- Improved logging and error handling in new endpoints for better debugging and compliance tracking.
- Updated server startup logs to include the new endpoints for easier reference.

The most important change is the addition of the GDPR consent logging endpoint because it ensures proper tracking and updating of user consents across multiple database tables, which is crucial for legal compliance and avoiding data processing issues for users who decline consent.
