...
Remember: It is better to return an error message than to generate a summary with insufficient or unreliable data.`;
     ...
     */
    async generateSummary(transcription, userId, incidentId) {
        try {
            this.logger.info(`📝 Generating strict AI summary for user: ${userId}`);

            // Validate transcription first
            const validation = this.validateTranscription(transcription);
            ...
            const response = await this.openai.chat.completions.create({
                ...
                messages: [
                    ...
                    {
                        role: 'user',
                        content: `Please analyze the following car accident transcription and provide a structured summary. Remember to use ONLY the facts explicitly stated in the transcription.
...
- Concrete facts identified: ${validation.factCount}
- Quality score: ${(validation.qualityScore * 100).toFixed(1)}%

Please provide the summary in the required format, or return an appropriate error message if the data is insufficient.`
                    }
                ]
            });
            ...
            if (summaryText.includes('Service is Unavailable') || 
                summaryText.includes('not currently enough data')) {
            ...
            if (this.supabase && userId) {
                ...
                if (error) {
                    ...
                    this.logger.success(`✅ Summary saved with ID: ${data.id}`);
                }
            }
...
class StrictAISummaryService {
  ...
  async generateSummary(transcriptionText, userId, incidentId) {
    try {
      ...
      // Validate and parse the summary
      const validation = this.validateSummary(aiSummary);
      ...
      const { data: savedSummary, error: saveError } = await this.supabase
        ...
        .insert({
          ...
          fact_count: validation.factCount,
          word_count: aiSummary.split(' ').length,
          validation_score: validation.score,
          created_at: new Date().toISOString(),
          metadata: {
            validation: validation,
            generation_version: '1.0.0',
            source_length: transcriptionText.length
          }
        })
        .select()
        .single();
      ...
      if (saveError) {
        this.logger.error('Failed to save AI summary:', saveError);
        throw new Error(`Database save failed: ${saveError.message}`);
      }

      this.logger.success(`AI summary generated and saved with ID: ${savedSummary.id}`);
      ...
      this.logger.error('AI summary generation error:', error);
      throw error;
    }
  }
    ...
    try {
      const response = await axios.post('https://api.openai.com/v1/chat/completions', {
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a legal expert who creates factual, structured incident summaries based only on provided information.'
          },
          {
            role: 'user',
            content: prompt
