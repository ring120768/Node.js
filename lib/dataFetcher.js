 ...
 * @param {string} createUserId - The user UUID
...
async function fetchAllData(createUserId, options = {}) {
  try {
    ...
    if (incidentId) {
      ...
      // SECURITY: Reject incident ID mismatch instead of falling back
      // If a specific incident was requested but doesn't belong to this user, abort
      if (!incidentData.length) {
        logger.error('🚨 SECURITY: Requested incident ID does not belong to user', {
          createUserId,
          incidentId,
          action: 'Aborting PDF generation'
        });
        throw new Error(`Incident ${incidentId} not found for user ${createUserId}`);
      }
    }
    ...
    // Fetch user documents (includes images from signup and incident photos)
    // CRITICAL FIX: Filter by incident_report_id to prevent fetching documents from OTHER incidents
    // We need BOTH:
    // 1. Signup documents (incident_report_id IS NULL) - driving license, vehicle photos from registration
    // 2. Incident documents (incident_report_id = THIS incident) - scene photos, damage photos from THIS crash
    let userDocumentsQuery = supabase
      .from('user_documents')
      .select('*')
      .eq('create_user_id', createUserId)
      .eq('status', 'completed')
    ...
    // Add incident filter if incident ID is provided
    if (incidentId) {
      userDocumentsQuery = userDocumentsQuery.or(`incident_report_id.eq.${incidentId},incident_report_id.is.null`);
    } else {
      // No incident ID provided - only fetch signup documents (null incident_report_id)
      userDocumentsQuery = userDocumentsQuery.is('incident_report_id', null);
    }

    const { data: userDocumentsData, error: userDocsError } = await userDocumentsQuery;
    ...
    if (incidentData && incidentData.length > 0) {
      ...
      // Try to find emergency audio linked to this incident
      let { data: emergencyAudio, error: audioError } = await supabase
      ...
      // If not found by incident_id, fall back to user's latest recording
      // (Emergency audio is recorded during form filling, before incident exists)
      if (!emergencyAudio) {
        console.log(`🔍 Emergency audio not found by incident_id, trying fallback by user_id: ${createUserId}`);
        ...
        emergencyAudio = fallbackQuery.data;
        audioError = fallbackQuery.error;

        if (emergencyAudio) {
          console.log(`✅ Emergency audio found via fallback: ${emergencyAudio.id} (recorded: ${emergencyAudio.recorded_at})`);
        }
      }
    ...
    const signedUrls = {};
    console.log(`🖼️  Processing ${Object.keys(userDocumentsByType).length} document types from user_documents table`);
    for (const [type, docs] of Object.entries(userDocumentsByType)) {
      // docs is now an ARRAY of documents with the same type
      const docsArray = Array.isArray(docs) ? docs : [docs];
      console.log(`   Processing type: ${type} (${docsArray.length} documents)`);
