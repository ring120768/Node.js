...
async function fetchAllData(createUserId, options = {}) {
  try {
    ...
    // CRITICAL FIX: Filter by incident_id to prevent fetching documents from OTHER incidents
    // We need BOTH:
    // 1. Signup documents (incident_id IS NULL) - driving license, vehicle photos from registration
    // 2. Incident documents (incident_id = THIS incident) - scene photos, damage photos from THIS crash
    ...
    if (incidentId) {
      userDocumentsQuery = userDocumentsQuery.or(`incident_id.eq.${incidentId},incident_id.is.null`);
    } else {
      // No incident ID provided - only fetch signup documents (null incident_id)
      userDocumentsQuery = userDocumentsQuery.is('incident_id', null);
    }
