...
async function fetchAllData(createUserId) {
  try {
    ...
    const allData = {
      ...
      aiTranscription: aiTranscriptionData, // NEW: AI transcription for PDF Page 14
      aiSummary: aiSummaryData, // NEW: AI summary for PDF Page 13
      images: imagesByType,
      ...
      metadata: {
        ...
        has_ai_transcription: !!aiTranscriptionData, // NEW
        has_ai_summary: !!aiSummaryData // NEW
      }
    };

    console.log(`✅ Data fetched successfully: ${allData.metadata.total_incidents} incidents, ${allData.metadata.total_images} legacy images, ${allData.metadata.total_user_documents} user documents, ${allData.metadata.total_witnesses} witnesses, ${allData.metadata.total_vehicles} vehicles, AI transcription: ${allData.metadata.has_ai_transcription ? 'YES' : 'NO'}, AI summary: ${allData.metadata.has_ai_summary ? 'YES' : 'NO'}`);
