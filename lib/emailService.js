 ...
 */
// NOTE: escapeHtml is already defined at module scope further down this file.
// It is a const arrow function, so it is initialised before any of these
// functions are ever CALLED - referencing it here is safe.

const ukDate = (d) => new Date(d).toLocaleDateString('en-GB');

/** One row of the subscription details table. */
function billingRow(label, value, colour) {
  return `                                            <tr>
                                                <td style="color: #666; font-size: 14px; padding: 5px 0;">
                                                    <strong>${escapeHtml(label)}:</strong>
                                                </td>
                                                <td style="color: ${colour || '#333'}; font-size: 14px; text-align: right; padding: 5px 0;">
                                                    ${escapeHtml(value)}
                                                </td>
                                            </tr>`;
}
 ...
 *
 * The template previously labelled the TRIAL end as "Renewal Date" and claimed
 * cover "for the next 12 months" beside a 7-day date. Those are now separate,
 * accurate lines: when the trial ends, when the first payment is taken and how
 * much, and that it renews annually thereafter.
 *
 * For family and business plans the email also carries the join code, since
 * that is where most people will look for it.
 *
 * @param {string} userEmail
 * @param {object} userData - { userName, subscriptionStartDate, trialEndsAt,
 *   firstPaymentDate, firstPaymentAmount, tier, isGroupPlan, joinCode, joinUrl,
 *   seatsUsed, seatsTotal, dashboardUrl }
 */
async function sendSubscriptionWelcome(userEmail, userData) {
  const {
    userName,
    subscriptionStartDate,
    subscriptionEndDate,      // retained for callers that still pass only this
    trialEndsAt = null,
    ...
    seatsUsed = null,
    seatsTotal = null,
    dashboardUrl = process.env.DASHBOARD_URL || 'https://www.carcrashlawyerai.com/dashboard.html'
  } = userData;

  // ---- billing rows: accurate about trial vs first payment vs renewal ----
  const rows = [];

  if (trialEndsAt) {
    rows.push(billingRow('Free trial ends', ukDate(trialEndsAt), '#0B7AB0'));
  }

  const paymentDate = firstPaymentDate || trialEndsAt;
  if (paymentDate) {
    rows.push(billingRow(
      trialEndsAt ? 'First payment' : 'Payment taken',
      firstPaymentAmount ? `${firstPaymentAmount} on ${ukDate(paymentDate)}` : ukDate(paymentDate)
    ));
  } else if (firstPaymentAmount) {
    rows.push(billingRow('Amount', firstPaymentAmount));
  }

  rows.push(billingRow('Renews', 'Annually', '#4CAF50'));

  if (tier) rows.push(billingRow('Plan', tier));

  // ---- opening sentence, matched to the actual state ----
  const planSummarySentence = trialEndsAt
    ? ` Your free trial runs until ${ukDate(trialEndsAt)}, and you can cancel any time before then at no charge.`
    : ' Your cover runs for a year and renews annually.';

  // ---- group invite block ----
  let groupSection = '';
  if (isGroupPlan && joinCode) {
    const word = String(tier || '').toLowerCase().includes('family') ? 'family' : 'team';
    const seats = (seatsUsed != null && seatsTotal != null)
      ? `${seatsUsed} of ${seatsTotal} seats used`
      : '';
    groupSection = `
                            <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border: 2px solid #0B7AB0; border-radius: 8px; margin: 25px 0;">
                                <tr>
                                    <td style="padding: 25px;">
                                        <h3 style="margin: 0 0 8px 0; color: #0B7AB0; font-size: 18px;">Invite your ${word}</h3>
                                        ${seats ? `<p style="margin: 0 0 12px 0; color: #155e75; font-size: 14px; font-weight: 600;">${escapeHtml(seats)}</p>` : ''}
                                        <p style="margin: 0 0 16px 0; color: #475569; font-size: 15px; line-height: 1.6;">
                                            Share this code with your ${word}. They enter it when they sign up &mdash; there is nothing more for them to pay.
                                        </p>
                                        <p style="margin: 0 0 16px 0; text-align: center; font-family: monospace; font-size: 24px; font-weight: bold; letter-spacing: 3px; color: #0f172a; background-color: #f8fafc; border: 2px dashed #94a3b8; border-radius: 8px; padding: 14px;">
                                            ${escapeHtml(joinCode)}
                                        </p>
                                        ${joinUrl ? `<p style="margin: 0 0 14px 0; text-align: center;">
                                            <a href="${escapeHtml(joinUrl)}" style="display: inline-block; background-color: #0B7AB0; color: #ffffff; text-decoration: none; padding: 12px 28px; border-radius: 8px; font-size: 15px; font-weight: 600;">Open the join page</a>
                                        </p>` : ''}
                                        <p style="margin: 0; color: #64748b; font-size: 13px; line-height: 1.5;">
                                            Anyone with this code can take a seat on your plan, so share it only with people you want covered. You can change the code any time from your dashboard.
                                        </p>
                                    </td>
                                </tr>
                            </table>`;
  ...
  const subject = isGroupPlan && joinCode
    ? 'Welcome to Car Crash Lawyer AI - your plan is active, invite your family'
  ...
  return sendTemplateEmail(
    ...
    {
      userName,
      subscriptionStartDate: ukDate(subscriptionStartDate),
      // kept so any other reference to it still resolves
      subscriptionEndDate: subscriptionEndDate ? ukDate(subscriptionEndDate) : '',
      planSummarySentence,
...
/**
 * Send review request email after PDF report delivery
 * @param {string} userEmail - User's email address
 * @param {object} userData - { userName }
 */
async function sendReviewRequestEmail(userEmail, userData) {
  const {
    userName = 'there',
  ...
  return sendTemplateEmail(
    userEmail,
    'How did we do? - Car Crash Lawyer AI',
    'review-request',
    {
      userName,
      dashboardUrl
...
function categoriseDocumentType(documentType) {
  ...
  // Dashcam footage (videos/photos from dashcam)
  if (type.includes('dashcam')) {
    return 'dashcam';
  }
...
async function sendImageDownloadLinks(supabaseClient, userId, userEmail, userName, incidentId = null) {
  try {
    ...
    let documentsQuery = supabaseClient
      .from('user_documents')
      .select('id, document_type, storage_path, file_extension, created_at, incident_report_id')
      .eq('create_user_id', userId)
      .eq('status', 'completed')
      .is('deleted_at', null)
      .order('created_at', { ascending: true });

    // CRITICAL FIX: Filter by incident_report_id if provided
    // Same logic as dataFetcher.js - get incident docs + signup docs (null incident_report_id)
    if (incidentId) {
      documentsQuery = documentsQuery.or(`incident_report_id.eq.${incidentId},incident_report_id.is.null`);
      console.log(`   🎯 Filtering images: incident ${incidentId} + signup docs (null incident_report_id)`);
    } else {
    ...
    const templateData = {
      ...
      // Category flags
      hasDashcamFootage: imageCategories.dashcam.length > 0,
      ...
      // Category HTML
      dashcamFootageHtml: buildImageLinksHtml(imageCategories.dashcam),
