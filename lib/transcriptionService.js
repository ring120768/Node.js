...
class TranscriptionService {
    ...
    async processTranscriptionFromBuffer(
        ...
        if (!userId) {
            this.logger.error('🚫 No user ID provided for transcription processing');
            throw new Error('User ID is required for transcription processing');
        }

        // Block problematic test user IDs
        const blockedUserIds = ['user_1759410448804_yzas7ml2p'];
        if (blockedUserIds.includes(userId)) {
            this.logger.error(`🚫 Blocked transcription for problematic user ID: ${userId}`);
            throw new Error('This user ID has been blocked from transcription processing');
        }

        // Block timestamp-based auto-generated user IDs
        if (userId && /^user_\d{13}_[a-z0-9]+$/.test(userId)) {
            this.logger.error(`🚫 Blocked transcription for auto-generated user ID: ${userId}`);
            throw new Error('Auto-generated user IDs are not allowed for transcription');
        }

        // Block temporary and development IDs
        if (userId.startsWith('temp_') || userId.startsWith('dev_') || userId.startsWith('user_')) {
            this.logger.error(`🚫 Blocked invalid user ID format: ${userId}`);
            throw new Error('Invalid user ID format. Only valid Typeform UUIDs are allowed.');
        }

        // CRITICAL: Validate ONLY Typeform UUIDs - NO EXCEPTIONS
        const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
        if (!uuidRegex.test(userId)) {
            this.logger.error(`🚫 REJECTED: Non-UUID user ID: ${userId} - MUST BE TYPEFORM UUID`);
            throw new Error('CRITICAL: User ID must be a valid UUID from Typeform. No dummy or generated IDs allowed.');
        }
        
        this.logger.info(`✅ Valid Typeform UUID accepted: ${userId.substring(0, 8)}...`);
