...
class TranscriptionService {
    ...
    async processTranscriptionFromBuffer(
        ...
        // CRITICAL FIX: Ensure userId is valid UUID format
        let dbUserId = userId;
        if (userId && !this.isValidUUID(userId)) {
            dbUserId = this.ensureValidUUID(userId);
            this.logger.info(`🔧 Converted user ID to UUID: ${userId} -> ${dbUserId}`);
        }
        ...
        try {
            ...
            if (this.supabase) {
                await this.saveTranscription(
                    dbUserId, // Use the UUID-formatted user ID
                    incidentId,
                    transcriptionText,
                    audioUrl,
                    queueId
            ...
            // Store in memory for WebSocket with correct user session tracking
            if (global.transcriptionStatuses) {
                global.transcriptionStatuses.set(queueId.toString(), {
                    status: 'COMPLETED',
                    transcription: transcriptionText,
                    error: null,
                    create_user_id: dbUserId, // Use UUID-formatted user ID
                    session_user_id: dbUserId, // Use UUID-formatted user ID
    ...
    /**
     * NEW: UUID validation and conversion utilities
     */
    isValidUUID(str) {
