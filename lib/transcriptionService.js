...
class TranscriptionService {
    ...
    async processTranscriptionFromBuffer(
        ...
        // Block temporary, test, or dummy IDs
        const blockedPrefixes = ['temp_', 'test_', 'dummy_', 'user_', 'dev_', 'mock_'];
        for (const prefix of blockedPrefixes) {
            if (userId.toString().toLowerCase().startsWith(prefix)) {
                throw new Error(`Invalid user ID format: ${userId} - temporary IDs not allowed`);
            }
        }
        ...
        if (!uuidRegex.test(userId)) {
            throw new Error(`Invalid user ID format: ${userId} - must be valid Typeform UUID`);
        }
