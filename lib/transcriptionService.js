...
class TranscriptionService {
    ...
    async processTranscriptionFromBuffer(
        ...
        // Block ALL non-Typeform user IDs with comprehensive validation
        const blockedPrefixes = [
            'temp_', 'test_', 'dummy_', 'user_', 'dev_', 'mock_', 
        ...
        for (const prefix of blockedPrefixes) {
            if (userIdStr.startsWith(prefix)) {
                throw new Error(`SECURITY VIOLATION: Blocked user ID format: ${userId} - only valid Typeform UUIDs allowed`);
            }
        }
        
        // Must be valid UUID v4 format from Typeform (stricter validation)
        const uuidV4Regex = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
        if (!uuidV4Regex.test(userId)) {
            throw new Error(`SECURITY VIOLATION: Invalid UUID format: ${userId} - must be valid Typeform UUID v4`);
        }
