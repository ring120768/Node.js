...
async function generatePDF(data) {
  try {
    ...
    // Page 13 - Voice Transcription, Analysis Metadata, Quality Review
    setFieldText('voice_transcription', aiAnalysis.voice_transcription || '');

    // Format analysis_metadata compactly for small text box
    const metadata = aiAnalysis.analysis_metadata || {};
    const metadataText = metadata.model && metadata.timestamp
      ? `Model: ${metadata.model} | Generated: ${new Date(metadata.timestamp).toLocaleString('en-GB')}${metadata.version ? ` | v${metadata.version}` : ''}`
      : '';
    setFieldText('analysis_metadata', metadataText);

    setFieldText('quality_review', aiAnalysis.quality_review || '');

    // Page 14 - AI Summary
    setFieldText('ai_summary', aiAnalysis.ai_summary || '');

    // Page 15 - Closing Statement
    setFieldText('closing_statement', aiAnalysis.closing_statement || '');

    // Page 16 - Final Review
    setFieldText('final_review', aiAnalysis.final_review || '');
