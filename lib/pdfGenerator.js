...
async function generatePDF(data) {
  try {
    ...
    setFieldText('emergency_contact_name', user.emergency_contact_name); // NEW: Emergency contact name
    setFieldText('emergency_contact_number', user.emergency_contact_number); // NEW: Emergency contact number
    ...
    // NEW: Extended Medical Information
    checkField('ambulance_callled', incident.ambulance_called === true); // Note: PDF has typo "callled"
    setFieldText('hospital_or_medical_center', incident.hospital_name || '');
    setFieldText('severity_of_injuries', incident.injury_severity || '');
    setFieldText('treatment_received_on_scene', incident.treatment_received || '');
    ...
    // NEW: Traffic Conditions (mutually exclusive - only one should be checked)
    checkField('traffic_conditions_heavy', incident.traffic_heavy === true);
    checkField('traffic_conditions_moderate', incident.traffic_moderate === true);
    checkField('traffic_conditions_light', incident.traffic_light === true);
    checkField('traffic_conditions_none', incident.traffic_none === true);

    // NEW: Road Markings (mutually exclusive)
    checkField('road_markings', incident.road_markings_yes === true);
    checkField('road_markings_partial_yes', incident.road_markings_partial === true);
    checkField('road_markings_no', incident.road_markings_no === true);
    ...
    // NEW: Witness 2 Information (from incident_witnesses table)
    setFieldText('witness_name_2', witness2.witness_2_name || '');
    setFieldText('witness_mobile_number_2', witness2.witness_2_mobile || '');
    setFieldText('witness_email_address_2', witness2.witness_2_email || ''); // FIXED: was witness_email_2
