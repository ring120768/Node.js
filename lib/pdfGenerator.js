...
// NEW: HTML rendering services for pages 13-16
const aiAnalysisHtmlRenderer = require('../src/services/aiAnalysisHtmlRenderer');
const htmlToPdfConverter = require('../src/services/htmlToPdfConverter');

/**
 * Generate PDF from collected data (HYBRID APPROACH)
 *
 * Flow:
 * 1. Fill PDF form for pages 1-12 (keep existing form-fill logic)
 * 2. Render HTML templates for pages 13-16 (beautiful styling from UI)
 * 3. Convert HTML to PDF using Puppeteer (with gradients & colors)
 * 4. Merge form-filled pages + HTML-rendered pages
 * 5. Fill remaining pages 17-18 as form fields
 *
...
async function generatePDF(data) {
  try {
    console.log('📝 Starting hybrid PDF generation (form + HTML)...');
    ...
    // CRITICAL: Prioritize currentIncident data over stale user_signup data (Page 1)
    // Users may update personal/vehicle details during incident flow
    const incident = data.currentIncident || {};

    // Fill Personal Information (Page 1)
    setFieldText('driver_name', incident.driver_name || user.name);
    setFieldText('surname', incident.surname || user.surname);
    setFieldText('email', incident.email || user.email);
    setFieldText('mobile', incident.mobile || user.mobile);
    setFieldText('street', incident.street_address || user.street_address);
    setFieldText('street_address_optional', incident.street_address_optional || user.street_address_optional);
    setFieldText('town', incident.town || user.town);
    setFieldText('postcode', incident.postcode || user.postcode);
    setFieldText('country', incident.country || user.country);
    setFieldText('driving_license_number', incident.driving_license_number || user.driving_license_number);
    setFieldText('date_of_birth', incident.date_of_birth || user.date_of_birth); // NEW: Driver date of birth (PDF field name)

    // Fill Vehicle Information (Page 1)
    setFieldText('license_plate', incident.car_registration_number || incident.vehicle_registration || user.car_registration_number || user.vehicle_registration);
    setFieldText('vehicle_make', incident.vehicle_make || user.vehicle_make);
    setFieldText('vehicle_model', incident.vehicle_model || user.vehicle_model);
    setFieldText('vehicle_colour', incident.vehicle_colour || user.vehicle_colour);
    setFieldText('vehicle_condition', incident.vehicle_condition || user.vehicle_condition);
    setFieldText('recovery_company', incident.recovery_company || user.recovery_company);
    setFieldText('recovery_breakdown_number', incident.recovery_breakdown_number || user.recovery_breakdown_number);
    setFieldText('recovery_breakdown_email', incident.recovery_breakdown_email || user.recovery_breakdown_email);

    // Fill Emergency Contact (Page 2)
    // CRITICAL: Prioritize incident emergency contact over user signup
    const emergencyContactSource = incident.emergency_contact || user.emergency_contact;
    if (emergencyContactSource) {
      // Parse pipe-delimited format: "Name | Phone | Email | Company"
      const parts = emergencyContactSource.split('|').map(p => p.trim());
      setFieldText('emergency_contact', emergencyContactSource);
    ...
    // CRITICAL: Prioritize currentIncident data (user may have updated during incident flow)
    setFieldText('insurance_company', incident.insurance_company || user.insurance_company);
    setFieldText('policy_number', incident.policy_number || user.policy_number || user.insurance_policy_number);
    setFieldText('policy_holder', incident.policy_holder || user.policy_holder);
    setFieldText('cover_type', incident.cover_type || user.cover_type);

    // Format signup date as DD/MM/YYYY for UK format (Page 2)
    ...
    if (signupDate) {
      ...
      // PDF field "subscription_start_date" shows when user opened account (Page 2)
    ...
    // ========================================
    // CRITICAL: Prioritize data.imageUrls (latest uploaded images) over stale user_signup columns
    // Image URLs from user_documents table (via imageUrls) take priority over legacy user_signup columns
    setFieldText('driving_license_picture', data.imageUrls?.driving_license || user.driving_license_picture || '');
    setFieldText('vehicle_picture_front', data.imageUrls?.vehicle_front || user.vehicle_picture_front || '');
    setFieldText('vehicle_picture_driver_side', data.imageUrls?.vehicle_driver_side || user.vehicle_picture_driver_side || '');
    setFieldText('vehicle_picture_passenger_side', data.imageUrls?.vehicle_passenger_side || user.vehicle_picture_passenger_side || '');
    setFieldText('vehicle_picture_back', data.imageUrls?.vehicle_back || user.vehicle_picture_back || '');

    // Fill Incident Report Data (Pages 4-17)
    // Note: incident variable already declared above (line 87)
    // AI analysis fields (Pages 13-16) are now in incident data
    const aiAnalysis = incident; // Use incident directly since AI fields are now in incident_reports table

    // Page 4 - Safety Assessment
    setFieldText('form_id', incident.id);
    setFieldText('submit_date', incident.created_at);

    // CRITICAL: Prioritize incident safety data over user_signup
    // Safety status may be updated during incident flow
    checkField('are_you_safe', (incident.are_you_safe ?? user.are_you_safe) === true);
    setFieldText('safety_status', incident.safety_status || user.safety_status || '');
    setFieldText('safety_status_timestamp', incident.safety_status_timestamp || user.safety_status_timestamp || '');
    // six_point_safety_check_completed is in incident_reports (boolean), fallback to user_signup.six_point_safety_check
    checkField('six_point_safety_check', (incident.six_point_safety_check_completed ?? user.six_point_safety_check) === true);
    setFieldText('six_point_safety_check_completed_at', incident.six_point_safety_check_completed_at || user.six_point_safety_check_completed_at || '');
    ...
    // Pages 11-12 - Evidence URLs (using correct PDF field names)
    ...
    // Multi-photo support: Vehicle damage photos (1-5)
    ...
    // Multi-photo support: Scene photos (1-3)
    ...
    // Multi-photo support: Other vehicle photos (1-3)
    ...
    // Pages 13-16 - AI Analysis (GPT-4o generated content)
    console.log('\n🔍 DEBUG - AI Analysis Data:', {
      hasData: !!aiAnalysis,
      voice_transcription: aiAnalysis.voice_transcription ? aiAnalysis.voice_transcription.substring(0, 50) : 'MISSING',
      analysis_metadata: aiAnalysis.analysis_metadata || 'MISSING',
      quality_review: aiAnalysis.quality_review ? 'EXISTS' : 'MISSING',
      ai_summary: aiAnalysis.ai_summary ? 'EXISTS' : 'MISSING',
      closing_statement: aiAnalysis.closing_statement ? 'EXISTS' : 'MISSING',
      final_review: aiAnalysis.final_review ? 'EXISTS' : 'MISSING'
    });

    // ========================================
    // PAGES 13-16: AI ANALYSIS (HTML → PDF)
    // ========================================
    // NEW: Render HTML templates with beautiful styling, then convert to PDF
    console.log('🎨 Rendering HTML templates for pages 13-16...');
    const htmlPages = await aiAnalysisHtmlRenderer.renderAllPages({
      voice_transcription: aiAnalysis.voice_transcription,
      analysis_metadata: aiAnalysis.analysis_metadata,
      quality_review: aiAnalysis.quality_review,
      ai_summary: aiAnalysis.ai_summary,
      closing_statement: aiAnalysis.closing_statement,
      final_review: aiAnalysis.final_review
    });
    ...
    // PDF field "Date69_af_date" shows submission date (Page 17)
    // Compare with subscription_start_date (Page 2) to detect same-day reactive use
    ...
    // Flatten the form first (pages 1-12, 17-18)
    form.flatten();
