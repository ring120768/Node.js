...
async function prepareAccidentDataForNarrative(userId, incidentId) {
  ...
  if (!uuidRegex.test(userId)) {
    Logger.critical(`SECURITY VIOLATION: Non-UUID user ID blocked: ${userId}`);
    throw new Error('SECURITY VIOLATION: Only valid Typeform UUIDs are permitted. No dummy, generated, or modified IDs allowed.');
  }
  
  // Additional security check - block known patterns
  if (userId.includes('temp_') || userId.includes('user_') || userId.includes('dummy_') || userId.includes('test_')) {
    Logger.critical(`SECURITY VIOLATION: Suspicious user ID pattern blocked: ${userId}`);
    throw new Error('SECURITY VIOLATION: Suspicious user ID pattern detected and blocked.');
  }
...
async function generateAISummary(transcription, userId, incidentId) {
  ...
  // CRITICAL: Block ANY invalid user ID pattern
  const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  if (!uuidRegex.test(userId)) {
  ...
  if (userId.includes('temp_') || userId.includes('user_') || userId.includes('dummy_') || userId.includes('test_') || userId.includes('mock_')) {
    Logger.critical(`SECURITY VIOLATION: Suspicious user ID pattern blocked: ${userId}`);
    throw new Error('SECURITY VIOLATION: Suspicious user ID pattern detected and blocked.');
  }
