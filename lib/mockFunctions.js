...
async function prepareAccidentDataForNarrative(userId, incidentId) {
  ...
  // NEVER generate, modify, or accept invalid user IDs
  
  if (!userId) {
    Logger.error('No user ID provided to prepareAccidentDataForNarrative');
    throw new Error('SECURITY: User ID is required and cannot be generated.');
  }
  
  // CRITICAL: Block ANY non-UUID format - NO EXCEPTIONS
  const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  if (!uuidRegex.test(userId)) {
    Logger.critical(`SECURITY VIOLATION: Non-UUID user ID blocked in prepareAccidentDataForNarrative: ${userId}`);
    Logger.critical(`CORRUPTION ALERT: Dummy ID generation attempt detected: ${userId}`);
    throw new Error('SECURITY VIOLATION: Only valid Typeform UUIDs are permitted. No dummy, generated, or modified IDs allowed.');
  }
  
  // Block ALL corruption patterns that indicate dummy/generated IDs
  const corruptionPatterns = [
    'temp_', 'user_', 'dummy_', 'test_', 'mock_', 'generated_', 'auto_', 
    'fake_', 'sample_', 'default_', 'placeholder_', 'demo_'
  ];
  
  for (const pattern of corruptionPatterns) {
    if (userId.includes(pattern)) {
      Logger.critical(`SECURITY VIOLATION: Corruption pattern '${pattern}' detected in prepareAccidentDataForNarrative: ${userId}`);
      Logger.critical(`BLOCKING: Dummy ID generation/usage attempt prevented`);
      throw new Error(`SECURITY VIOLATION: Corruption pattern '${pattern}' detected. Only pure Typeform UUIDs allowed.`);
    }
  }
  
  // Block timestamp-based patterns
  if (/\d{13}/.test(userId)) {
    Logger.critical(`SECURITY VIOLATION: Timestamp pattern in prepareAccidentDataForNarrative: ${userId}`);
    throw new Error('SECURITY VIOLATION: Auto-generated timestamp IDs not allowed. Only Typeform UUIDs permitted.');
  }
  
  // NEVER call mock functions - always throw error
  Logger.critical('BLOCKED: Mock data preparation function called in production');
  throw new Error('SECURITY VIOLATION: Mock data preparation is permanently disabled. System requires real database access with valid Typeform UUID.');
}
...
async function generateAISummary(transcription, userId, incidentId) {
  ...
  if (!userId) {
    Logger.critical('SECURITY VIOLATION: No user ID provided to generateAISummary');
    throw new Error('SECURITY VIOLATION: User ID is required and cannot be generated.');
  }
  
  // CRITICAL: Block ANY invalid user ID pattern - MUST BE VALID UUID FROM TYPEFORM
  const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  if (!uuidRegex.test(userId)) {
    Logger.critical(`SECURITY VIOLATION: Invalid user ID format blocked in generateAISummary: ${userId}`);
    Logger.critical(`CORRUPTION ALERT: Non-UUID user ID detected: ${userId}`);
    throw new Error('SECURITY VIOLATION: Only valid Typeform UUIDs are permitted. No dummy, generated, temp, or modified IDs allowed.');
  }
  
  // Block ANY suspicious patterns that indicate dummy/generated IDs
  const corruptionPatterns = [
    'temp_', 'user_', 'dummy_', 'test_', 'mock_', 'generated_', 'auto_', 
    'fake_', 'sample_', 'default_', 'placeholder_', 'demo_'
  ];
  ...
  for (const pattern of corruptionPatterns) {
    if (userId.includes(pattern)) {
      Logger.critical(`SECURITY VIOLATION: Corruption pattern '${pattern}' detected in user ID: ${userId}`);
      Logger.critical(`BLOCKING: Dummy ID generation attempt detected and prevented`);
      throw new Error(`SECURITY VIOLATION: Corruption pattern '${pattern}' detected in user ID. Only pure Typeform UUIDs allowed.`);
    }
  }
  
  // Additional check for timestamp-based IDs (often auto-generated)
  if (/\d{13}/.test(userId)) {
    Logger.critical(`SECURITY VIOLATION: Timestamp pattern detected in user ID: ${userId} - likely auto-generated`);
    throw new Error('SECURITY VIOLATION: Timestamp-based IDs not allowed. Only Typeform UUIDs permitted.');
  }
