...
async function prepareAccidentDataForNarrative(userId, incidentId) {
  ...
  // CRITICAL: Validate Typeform UUID format only - NO EXCEPTIONS
  const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  if (!uuidRegex.test(userId)) {
    Logger.error(`REJECTED: Non-UUID user ID detected: ${userId} - MUST BE FROM TYPEFORM`);
    throw new Error('CRITICAL: Invalid user ID format. Only valid Typeform UUIDs are allowed. No dummy or generated IDs permitted.');
  }
  
  // NEVER call mock functions - always throw error
  Logger.critical('BLOCKED: Mock function attempted to access data preparation');
  throw new Error('SECURITY VIOLATION: Mock data preparation blocked. System requires real Typeform UUID.');
}
